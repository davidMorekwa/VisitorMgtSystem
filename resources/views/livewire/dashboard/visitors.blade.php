<div class="flex flex-row">
			...
			<tbody>
				...
				<tr wire:click='handleVisitorClick({{ $visitor->id }});'
					...
					<td class="flex flex-row items-center justify-center">
						...
						{{-- edit button --}}
						<button wire:click='handleEditClick' id="{{ $visitor->id }}"
						...
						{{-- delete button --}}
						<form action="/dashboard/visitors/delete" method="POST" class="flex items-center">
							<input name="visitor_id" type="number" value="{{ $visitor->id }}" hidden>
							@method('DELETE')
							@csrf
							<button type="submit" {{-- wire:click='handleDeleteClick({{ $visitor->id }})'
								--}}>
								<svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="24"
									height="24" viewBox="0 0 64 64">
									<path
	...
	@if ($selected_visitor != '')
	<div class="w-2/5 border-2 border-r-red-800">
		<div>
		...
		<br>
		<div>
			<x-button>Generate Report</x-button>
		</div>
	</div>
	@endif
</div>
